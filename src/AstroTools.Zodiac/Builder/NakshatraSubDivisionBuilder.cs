...
{
    public const string SubLords = "SubLords";
    ...
    private SubDivision CreateSubDivisions(Planet lord, Degree startDeg, string name, Degree region)
    {
        var planets = ReorderPlanets(lord);

        var acc = startDeg;

        return new SubDivision("SubLord", planets
            .Select(p =>
            {
    ...
    public Dictionary<string, SubDivision> BuildSubDivisions(Nakshatra nakshatra)
    {
        return new Dictionary<string, SubDivision>
        {
            { "SubLords", CreateSubDivisions(nakshatra.Lord, nakshatra.Region.Start, "SubLord", StarRegion) }
        };
    }
}